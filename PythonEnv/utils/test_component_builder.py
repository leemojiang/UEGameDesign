...
class ComponentBuilder:
    ...
    def add_component(self,parent_handler:unreal.SubobjectDataHandle ,blueprint: unreal.Blueprint, new_class, name: str) -> (unreal.SubobjectDataHandle, unreal.Object):
        ...
        self.SDS.attach_subobject(owner_handle=parent_handler, child_to_add_handle=sub_handle)

        obj: unreal.Object = self._get_component_object(sub_handle, blueprint)
        return sub_handle, obj
    
    def get_component(self,blueprint:unreal.Blueprint, name:str, comp_type:Optional[unreal.Class]=None)->(unreal.SubobjectDataHandle, unreal.Object):
            ...
            if self.BFL.is_handle_valid(h):
                ...
                if var_name == name:
                    ...
                    if comp_type is None or isinstance(obj,comp_type):
                        print(f"Component with name {name} and type {comp_type} found! ")
                        result = [h, obj]
        if result is None:         
    ...
    def get_components_of_type(self,blueprint:unreal.Blueprint, comp_type:unreal.Class)->list[(unreal.SubobjectDataHandle, unreal.Object)]:
        result = []
        handles = self._get_subobject_handles(blueprint)
        for h in handles:
            if self.BFL.is_handle_valid(h):
    ...
    def print_component_info_from_handle(self,h):
        if self.BFL.is_handle_valid(h):
            subdata = self.BFL.get_data(h)
            ...
            print(f"   Is Component: {is_component}, Root Component: {is_root}, Scene Component: {is_scene}, Native Component: {is_native}, Inherited Component: {is_inherited} \n")
    ...
    def print_components_info(self,handles,blueprint: Optional[unreal.Blueprint|unreal.Actor]=None):
        for h in handles:
            if self.BFL.is_handle_valid(h):
                subdata = self.BFL.get_data(h)
                comp_obj = self.BFL.get_object(subdata)
