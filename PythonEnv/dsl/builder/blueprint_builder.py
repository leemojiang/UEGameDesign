...
class BlueprintBuilder:
    ...
    def create_blueprint_from_model(self, model: ActorModel, new_asset_path: str ,overwrite=False):
        if not new_asset_path.endswith("/"):
            raise ValueError("new_asset_path 必须是以 / 结尾的路径")
        
        new_asset_path += model.name

        if self.asset_exists(new_asset_path):
            if not overwrite:
                # raise FileExistsError(f"资产已存在: {new_asset_path}")
                print(f"资产已存在: {new_asset_path}")
                return unreal.load_object(None,new_asset_path),new_asset_path
        ...
        return blueprint,new_asset_path

    def tweak_blueprint_from_model(self, model: ActorModel, asset_path: str):
        # 加载 BlueprintGeneratedClass
        bp = unreal.load_object(None,asset_path)
        # ComponentBuilder needs BPasset rather BPgeneratedClass
        bp_asset = unreal.BlueprintEditorLibrary.get_blueprint_asset(bp)
        if bp_asset is None:
            raise RuntimeError(f"无法加载 BlueprintAsset: {asset_path}")
        ...
        for comp in model.components:
            ...
            apply_properties(comp_obj,properties_model.model_dump(serialize_as_any=True,exclude_none=True)) #comp 
        # SceneComponent
        for sc in model.children:
            self._build_scene_component(sc, bp_asset, root_handle)
        
        # Actor 
        bp_class = get_unreal_class(asset_path)
        cdo = unreal.get_default_object(bp_class)
        apply_properties(cdo,model.properties.model_dump(serialize_as_any=True,exclude_none=True))
